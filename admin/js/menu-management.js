...
async function initMenuManagement() {
  ...
  document.querySelector("#item-image-file").addEventListener("change", uploadItemImage);
}
...
async function uploadItemImage(e) {
  ...
  if (!file) return;
  try {
    const url = await uploadToVelouraStorage(file, "menu");
    document.querySelector("#item-image").value = url;
